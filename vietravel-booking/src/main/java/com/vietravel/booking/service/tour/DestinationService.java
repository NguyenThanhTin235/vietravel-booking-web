...
public class DestinationService{
    ...
    public DestinationService(DestinationRepository destinationRepository,TourCategoryRepository tourCategoryRepository){
    ...
    private void apply(Destination d,DestinationUpsertRequest req){
        ...
        if(type==null) type=DestinationType.CITY;
        d.setType(type);

        String slug=req.getSlug();
        if(slug==null||slug.trim().isEmpty()) slug=toSlug(req.getName());
        d.setSlug(slug.trim());
    ...
    private void validate(DestinationUpsertRequest req,Long currentId){
        if(req.getName()==null||req.getName().trim().isEmpty()) throw new RuntimeException("Tên điểm đến không được rỗng");
        if(req.getCategoryId()==null) throw new RuntimeException("Vui lòng chọn danh mục");
