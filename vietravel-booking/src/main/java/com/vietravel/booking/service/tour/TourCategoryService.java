...
public class TourCategoryService{
    ...
    public TourCategoryResponse create(TourCategoryUpsertRequest req){
        normalize(req);
        if(repo.existsBySlug(req.getSlug())) throw new RuntimeException("Slug đã tồn tại");
    ...
    public TourCategoryResponse update(Long id,TourCategoryUpsertRequest req){
        ...
        repo.findBySlug(req.getSlug()).ifPresent(exist->{
            if(!exist.getId().equals(id)) throw new RuntimeException("Slug đã tồn tại");
        });
    ...
    public void delete(Long id){
        if(!repo.existsById(id)) throw new RuntimeException("Không tìm thấy danh mục");
        repo.deleteById(id);
    }

    private void normalize(TourCategoryUpsertRequest req){
        if(req==null) throw new RuntimeException("Dữ liệu không hợp lệ");
        if(req.getName()==null||req.getName().trim().isEmpty()) throw new RuntimeException("Tên không được rỗng");
    ...
    private TourCategory resolveParent(Long parentId,Long selfId){
        ...
        if(selfId!=null&&parentId.equals(selfId)) throw new RuntimeException("Danh mục cha không hợp lệ");
        TourCategory p=repo.findById(parentId).orElseThrow(()->new RuntimeException("Danh mục cha không tồn tại"));

        if(selfId!=null){
            TourCategory cur=p;
            while(cur!=null){
                if(cur.getId().equals(selfId)) throw new RuntimeException("Không được chọn cha là con của chính nó");
                cur=cur.getParent();
            }
        }
        return p;
    ...
    private String slugify(String s){
        ...
        v=v.replaceAll("(^-+)|(-+$)","");
        v=v.replaceAll("-+","-");
        if(v.isEmpty()) throw new RuntimeException("Slug không hợp lệ");
        return v;
    }
}
