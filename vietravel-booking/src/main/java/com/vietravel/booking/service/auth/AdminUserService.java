...
public class AdminUserService{
    ...
    public AdminUserDetailResponse get(Long id){
        UserAccount u=userAccountRepository.findByIdWithProfile(id).orElseThrow(()->new RuntimeException("Không tìm thấy người dùng"));
        return toDetail(u);
    }

    @Transactional
    public AdminUserDetailResponse createStaff(AdminStaffCreateRequest req){
        if(req.getEmail()==null||req.getEmail().trim().isEmpty()) throw new RuntimeException("Email không được rỗng");
        if(req.getPassword()==null||req.getPassword().trim().isEmpty()) throw new RuntimeException("Mật khẩu không được rỗng");
        if(req.getFullName()==null||req.getFullName().trim().isEmpty()) throw new RuntimeException("Họ tên không được rỗng");

        String email=req.getEmail().trim().toLowerCase();
        userAccountRepository.findByEmail(email).ifPresent(x->{throw new RuntimeException("Email đã tồn tại");});
    ...
    public AdminUserRowResponse toggleLock(Long id){
        ...
        else u.setStatus(UserStatus.LOCKED);
        return toRow(u);
    }
