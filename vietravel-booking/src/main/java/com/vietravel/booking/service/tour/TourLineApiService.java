...
public class TourLineApiService{
    ...
    private void validateUpsert(TourLineUpsertRequest req){
        if(req==null) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Dữ liệu không hợp lệ");
        if(req.getCode()==null||req.getCode().trim().isEmpty()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"code không được rỗng");
        if(req.getName()==null||req.getName().trim().isEmpty()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"name không được rỗng");
        if(req.getMinPrice()==null||req.getMaxPrice()==null) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"minPrice/maxPrice không được rỗng");
        if(req.getMinPrice().compareTo(BigDecimal.ZERO)<0||req.getMaxPrice().compareTo(BigDecimal.ZERO)<0) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"giá không hợp lệ");
        if(req.getMinPrice().compareTo(req.getMaxPrice())>0) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"minPrice phải <= maxPrice");
    ...
    public List<TourLineResponse> list(Boolean onlyActive){
        List<TourLine> items=onlyActive!=null&&onlyActive?repo.findByIsActiveTrueOrderBySortOrderAsc():repo.findAll();
        return items.stream().map(this::toRes).toList();
    }

    public TourLineResponse get(Long id){
        TourLine e=repo.findById(id).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"Không tìm thấy tour line"));
        return toRes(e);
    }
    ...
    public void delete(Long id){
        if(!repo.existsById(id)) throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Không tìm thấy tour line");
        repo.deleteById(id);
    }

    public TourLineResponse resolveByPrice(BigDecimal price){
        if(price==null) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"price không được rỗng");
        return repo.findFirstByIsActiveTrueAndMinPriceLessThanEqualAndMaxPriceGreaterThanEqual(price,price)
                .map(this::toRes)
                .orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"Không có tour line phù hợp với giá"));
    }
}
